...
static int hls_read_header(AVFormatContext *s)
{
    ...
    for (i = 0; i < c->n_playlists; i++) {
        ...
        av_log(s, AV_LOG_WARNING, "segment[0] url:%s, name:%s\r\n",
            pls->segments[0]->url, in_fmt->name);
...
static int hls_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    //av_log(s, AV_LOG_WARNING, "pls count:%d.\r\n", c->n_playlists);
    for (i = 0; i < c->n_playlists; i++) {
        struct playlist *pls = c->playlists[i];
        /* Make sure we've got one buffered packet from each open playlist
         * stream */
        //av_log(s, AV_LOG_WARNING, "pls index:%d, url:%s.\r\n", i, pls->url);
        if (pls->needed && !pls->pkt.data) {
            while (1) {
                ...
                if (ret < 0) {
                    //av_log(s, AV_LOG_WARNING, "av_read_frame return ret:%d, error:%s.\r\n", ret, av_err2str(ret));
...
static const AVOption hls_options[] = {
    ...
    {"allowed_extensions", "List of file extensions that hls is allowed to access",
        ...
        {.str = "flv,3gp,aac,avi,flac,mkv,m3u8,m4a,m4s,m4v,mpg,mov,mp2,mp3,mp4,mpeg,mpegts,ogg,ogv,oga,ts,vob,wav"},
