...
static int mov_read_tfdt(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!st) {
        av_log(c->fc, AV_LOG_ERROR, "tfdt could not find corresponding track id %u\n", frag->track_id);
        return AVERROR_INVALIDDATA;
    }
    sc = st->priv_data;
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!st) {
        av_log(c->fc, AV_LOG_ERROR, "trun could not find corresponding track id %u\n", frag->track_id);
        return AVERROR_INVALIDDATA;
    }
    sc = st->priv_data;
    if (sc->pseudo_stream_id+1 != frag->stsd_id && sc->pseudo_stream_id != -1)
...
static int mov_read_sidx(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (!st) {
        av_log(c->fc, AV_LOG_WARNING, "sidx could not find corresponding track id %d\n", track_id);
        return 0;
    }
